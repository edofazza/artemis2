...
class DistillationExecutor(object):
    ...
    def _train_epoch(self, epoch):
        ...
        for data, summaries_align, summaries_flava, label in self.train_loader:
            data, summaries_align, summaries_flava, label = (data.to(self.gpu_id), summaries_align.to(self.gpu_id),
                                                             summaries_flava.to(self.gpu_id), label.long().to(self.gpu_id))
            self.optimizer.zero_grad()
            with torch.no_grad():
                teacher_logits = self.teacher_model.ensemble(
                    data, summaries_align, summaries_flava, [0.043700000830110684, 0.557841995937574, 0.39845800323231534])
    ...
    def test(self):
        ...
        for data, _, _, label in self.test_loader:
