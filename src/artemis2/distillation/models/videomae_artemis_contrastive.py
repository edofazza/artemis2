...
from transformers import (VideoMAEModel, CLIPTokenizer, CLIPTextModel,
                          CLIPVisionModel, logging, Dinov2Model, Dinov2WithRegistersModel,
                          IJepaModel, FlavaImageModel, AlignVisionModel, GroupViTVisionModel,
                          Owlv2VisionModel, AlignTextModel, GroupViTTextModel, FlavaTextModel,
                          AutoTokenizer)
...
class VideoMAEArtemisContrastive(nn.Module):
    def __init__(self, class_embed, num_frames, recurrent: str | None = None, fusion: str = 'normal',
        ...
        super(VideoMAEArtemisContrastive, self).__init__()
        ...
        # BRANCH 1
        self.backbone = VideoMAEModel.from_pretrained("MCG-NJU/videomae-base-finetuned-kinetics", num_frames=num_frames,
                                                      ignore_mismatched_sizes=True)
        self.linear1 = nn.Linear(in_features=self.backbone.config.hidden_size + self.embed_dim,
        ...
        if self.recurrent == 'bilstm':
            self.bilstm = nn.LSTM(input_size=self.embed_dim, hidden_size=self.embed_dim // 2, num_layers=1,
                                  bidirectional=True)
        elif self.recurrent == 'gru':
            self.gru = nn.GRU(input_size=self.embed_dim, hidden_size=self.embed_dim // 2, num_layers=1,
                              bidirectional=True)
        ...
        if self.backbone_residual:
            self.linear_backbone_residual = nn.Linear(in_features=self.backbone.config.hidden_size,
        ...
        if self.image_residual:
            if image_encoder == 'align':
                self.linear_image_residual = nn.Linear(in_features=640, out_features=self.embed_dim)  # ALIGN
            elif image_encoder == 'groupvit':
                self.linear_image_residual = nn.Linear(in_features=384, out_features=self.embed_dim)  # ALIGN
            else:
                self.linear_image_residual = nn.Linear(in_features=self.backbone.config.hidden_size,
                                                       out_features=self.embed_dim)
        ...
        self.image_model = Dinov2Model.from_pretrained("facebook/dinov2-base")
        # self.image_model = Dinov2WithRegistersModel.from_pretrained("facebook/dinov2-with-registers-base")
        # self.image_model = IJepaModel.from_pretrained("facebook/ijepa_vith14_1k") # [1, 197, 768]
        # self.image_model = AlignVisionModel.from_pretrained("kakaobrain/align-base") # [1, 197, 768]
        # self.image_model = FlavaImageModel.from_pretrained("facebook/flava-full") # [1, 197, 768]
        # self.image_model = GroupViTVisionModel.from_pretrained("nvidia/groupvit-gcc-yfcc")
        # self.image_model = Owlv2VisionModel.from_pretrained("google/owlv2-base-patch16")

        if image_encoder == 'align':
            self.linear2 = nn.Linear(in_features=640 + self.embed_dim,
                                     out_features=self.embed_dim, bias=False)
        elif image_encoder == 'groupvit':
            self.linear2 = nn.Linear(in_features=384 + self.embed_dim,
                                     out_features=self.embed_dim, bias=False)
        else:
        ...
        if self.fusion == 'crossattention':
            self.cross_attention = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=8, dropout=0.1,
                                                         batch_first=True)
            self.linear_fusion = nn.Linear(in_features=16, out_features=self.num_classes)
        if self.fusion == 'selfattention':
            self.cross_attention_summary = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=4, dropout=0.1,
                                                                 batch_first=True)
            self.cross_attention_video = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=4, dropout=0.1,
                                                               batch_first=True)
            ...
            self.cross_attention = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=4, dropout=0.1,
                                                         batch_first=True)
    ...
    def forward(self, images, summaries):
        ...
        x = self.linear1(
            torch.cat((
                ...
                # torch.randn(1, 16, x.shape[2]).to('cuda', non_blocking=True), # TEST FVCORE
        ...
        """video_features = self.image_model(images.reshape(b * t, c, h, w))
        #print(video_features.shape, flush=True)
        print(video_features.last_hidden_state.shape, flush=True)
        print(video_features.last_hidden_state.reshape(b, t, -1).shape, flush=True)
        print(video_features.last_hidden_state.reshape(b, t, -1).mean(dim=1, keepdim=True).shape, flush=True)
        video_features = video_features.last_hidden_state.reshape(b, t, -1).mean(dim=1, keepdim=True)"""
...
class VideoMAEArtemisContrastiveExecutor(object):
    def __init__(self, train_loader, test_loader, criterion, eval_metric, class_list, test_every, distributed,
                 ...
                 backbone_residual: bool, linear2_residual, image_residual, contrastive: str) -> None:
        super().__init__()
        ...
        model = (VideoMAEArtemisContrastive(class_embed, num_frames, recurrent, fusion, residual, relu,
                                            summary_residual, backbone_residual, linear2_residual, image_residual)
                 .to(gpu_id))
        if distributed:
    ...
    def _get_text_features(self, cl_names):
        ...
        # text_model = GroupViTTextModel.from_pretrained("nvidia/groupvit-gcc-yfcc")

        text_model = AlignTextModel.from_pretrained("kakaobrain/align-base")
    ...
    def train(self, start_epoch, end_epoch):
        ...
        for epoch in range(start_epoch, int(end_epoch * 0.5)):
