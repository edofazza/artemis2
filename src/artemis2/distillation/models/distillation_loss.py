...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, video_embed: torch.Tensor, text_embed: torch.Tensor) -> torch.Tensor:
        ...
        scores = torch.matmul(video_embed, text_embed.T)    # shape: (batch_size, batch_size)

        logit_scale = self.logit_scale.exp()
        logits_per_video = logit_scale * scores
...
class MultilabelDistillationLossWithDynamicTemperature(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, true_labels):
        ...
        # Use mean squared error (MSE) as a proxy for KL divergence in multilabel distillation
        #distill_loss = F.mse_loss(student_probs, teacher_probs)
        distill_loss = self.info_loss(student_probs, teacher_probs)
