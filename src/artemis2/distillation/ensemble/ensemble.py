...
class Ensemble(object):
    ...
    def ensemble(self, data, summaries_align, summaries_flava, weights: list):
        assert len(weights) == len(self.models), 'Number of weights must equal number of models'
        for model in self.models:
            model.eval()
        weights = [w / sum(weights) for w in weights]
        with torch.no_grad():
            tmp_results = self.models[0](data, summaries_align)
            if type(tmp_results) is tuple:
                tmp_results = tmp_results[0]
            weighted_predictions = tmp_results * weights[0]
            for i in range(1, len(self.models)):
                if i == 1:
                    tmp_results = self.models[i](data, summaries_flava)
                else:
                    tmp_results = self.models[i](data, summaries_align)
                if type(tmp_results) is tuple:
                    tmp_results = tmp_results[0]
