...
from models.vjepa.utils import init_model


class VJEPAARTEMIS(nn.Module):
    def __init__(self, class_embed, device, pretrained_path, config, recurrent: str | None = None,
                 fusion: str = 'normal', residual: bool = False, relu: bool = False, summary_residual: bool = False,
                 backbone_residual: bool = False, linear2_residual: bool = False, image_residual: bool = False,):
        super(VJEPAARTEMIS, self).__init__()
        ...
        uniform_power = cfgs_model.get('uniform_power')

        cfgs_meta = args.get('meta')
        use_sdpa = cfgs_meta.get('use_sdpa')

        self.backbone = init_model(
            crop_size=crop_size,
            device=device,
        ...
        self.linear1 = nn.Linear(in_features=1024 + self.embed_dim, out_features=self.embed_dim, bias=False)    # ADAPTED TO VJEPA
        ...
        if self.backbone_residual:
            self.linear_backbone_residual = nn.Linear(in_features=1024, out_features=self.embed_dim)  # ADAPTED TO VJEPA
        ...
        if self.image_residual:
            self.linear_image_residual = nn.Linear(in_features=768, out_features=self.embed_dim)
    ...
    def forward(self, images, summaries):
        ...
        x = self.backbone(images.reshape(b, c, t, h, w))    # (b, 1568, 1024)
...
class VJEPAARTEMISExecutor(object):
    def __init__(self, train_loader, test_loader, criterion, eval_metric, class_list, test_every, distributed,
        ...
        model = VJEPAARTEMIS(class_embed, self.gpu_id, 'vitl16.pth.tar', 'vitl16.yaml', recurrent,
                             fusion, residual, relu, summary_residual, backbone_residual, linear2_residual,
                             image_residual).to(gpu_id)
        if distributed:
