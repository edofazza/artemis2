...
class VideoMAEArtemis(nn.Module):
    def __init__(self, class_embed, num_frames, recurrent: str | None = None, fusion: str = 'normal',
        ...
        super(VideoMAEArtemis, self).__init__()
...
class VideoMAEArtemisExecutor(object):
    def __init__(self, train_loader, test_loader, criterion, eval_metric, class_list, test_every, distributed,
        ...
        model = VideoMAEArtemis(class_embed, num_frames, recurrent, fusion, residual, relu,
    ...
    def load(self, file_path):
        ...
        # Get your model's current state dict
        model_state = self.model.state_dict()

        # Create a new state dict with only matching keys and shapes
        filtered_checkpoint = {}
        mismatched_keys = []  # to keep track of which keys were not loaded

        for key, value in checkpoint.items():
            if key in model_state:
                if value.size() == model_state[key].size():
        ...
        print("Keys with mismatched shapes (will be randomly initialized):", mismatched_keys)

        # Update your model's state dict with the filtered checkpoint
        ...
        # Unfreeze the mismatched parts so they are trainable.
        # You can manually unfreeze based on the key names.
        # For example:
        if "query_embed" in mismatched_keys or any(k.startswith("query_embed") for k in mismatched_keys):
            self.model.query_embed.requires_grad = True

        if any(k.startswith("linear_residual") for k in mismatched_keys):
            for param in self.model.linear_residual.parameters():
                param.requires_grad = True

        if any(k.startswith("group_linear") for k in mismatched_keys):
            for param in self.model.group_linear.parameters():
                param.requires_grad = True"""
