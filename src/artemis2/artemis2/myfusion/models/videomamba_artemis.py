...
class VideoMambaArtemis(nn.Module):
    def __init__(self, class_embed, num_frames, recurrent: str | None = None, fusion: str = 'normal',
                 ...
                 version: str = 't'):
        super(VideoMambaArtemis, self).__init__()
        ...
        if version == 'm':
            self.linear1 = nn.Linear(in_features=576 + self.embed_dim, out_features=self.embed_dim, bias=False)
        elif version == 's':
            self.linear1 = nn.Linear(in_features=384 + self.embed_dim, out_features=self.embed_dim, bias=False)
        elif version == 't':
        ...
        if self.backbone_residual:
            if version == 'm':
                self.linear_backbone_residual = nn.Linear(in_features=576, out_features=self.embed_dim)
            elif version == 's':
                self.linear_backbone_residual = nn.Linear(in_features=384, out_features=self.embed_dim)
            elif version == 't':
                self.linear_backbone_residual = nn.Linear(in_features=192, out_features=self.embed_dim)
...
class VideoMambaArtemisExecutor(object):
    def __init__(self, train_loader, test_loader, criterion, eval_metric, class_list, test_every, distributed,
        ...
        model = VideoMambaArtemis(class_embed, num_frames, recurrent, fusion, residual, relu,
