...
class CCALoss(nn.Module):
    ...
    def forward(self, features1, features2):
        ...
        # Compute CCA
        cca = CCA(n_components=self.num_components)
        cca.fit(features1.cpu().numpy(), features2.cpu().numpy())
        transformed_features1 = cca.transform(features1.cpu().numpy())
        transformed_features2 = cca.transform(features2.cpu().numpy())

        # Compute loss
        correlation = np.mean(np.corrcoef(transformed_features1.T, transformed_features2.T)[:self.num_components, self.num_components:])
        loss = 1 - correlation  # Loss = 1 - correlation
        return torch.tensor(loss, dtype=torch.float32, device=features1.device)


class MambaMSQNetVariation1(nn.Module):
    def __init__(self, class_embed, num_frames, recurrent: str | None = None, fusion: str = 'normal',
        ...
        super(MambaMSQNetVariation1, self).__init__()
        ...
        self.linear1 = nn.Linear(in_features=192 + self.embed_dim, out_features=self.embed_dim, bias=False)
        ...
        if self.recurrent == 'bilstm':
            ...
            self.conv = nn.Conv1d(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1)
...
class MambaMSQNetVariation1Executor(object):
    def __init__(self, train_loader, test_loader, criterion, eval_metric, class_list, test_every, distributed,
        ...
        #cosine_similarity_loss_fn = CosineSimilarityLoss(margin=0.0)
        #cca_loss_fn = CCALoss(num_components=10)
        ...
        model = MambaMSQNetVariation1(class_embed, num_frames, None, 'normal', True, False,
                                 False, False, False, False).to(gpu_id)
        if distributed:
    ...
    def _train_batch(self, data, summaries, label):
        ...
        loss_this.backward()
        self.optimizer.step()
        return loss_this.item(), 0 #contrastive_loss.item()
...
if __name__ == '__main__':
    ...
    model = MSQNetVariation1(torch.rand((140, 512), ), 16)
