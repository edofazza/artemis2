...
from .datasets import AnimalKingdom, Charades, Hockey, Thumos14, Volleyball, BaboonLandDataset, MammalNet, RobotObservations
...
class DataManager():
    ...
    def _check(self, ):
        datasets_list = ["animalkingdom", "baboonland", "ava", "charades", "hockey", "thumos14", "volleyball",
                         'mammalnet', 'robot']
    ...
    def get_act_dict(self, ):
        ...
        hockey_dict = {'Celebration': 0, 'Checking': 1, 'Corner Action': 2, 'End of Period': 3, 'Face-Off': 4,
                       'Fight': 5, 'Goal': 6, 'Line Change': 7, 'Penalty': 8, 'Shot': 9, 'Save': 10, 'Play': 11}
        thumos14_dict = {'BaseballPitch': 0, 'BasketballDunk': 1, 'Billiards': 2, 'CleanAndJerk': 3, 'CliffDiving': 4,
                         'CricketBowling': 5, 'CricketShot': 6, 'Diving': 7, 'FrisbeeCatch': 8, 'GolfSwing': 9,
                         'HammerThrow': 10, 'HighJump': 11, 'JavelinThrow': 12, 'LongJump': 13, 'PoleVault': 14,
                         'Shotput': 15, 'SoccerPenalty': 16, 'TennisSwing': 17, 'ThrowDiscus': 18,
                         'VolleyballSpiking': 19}
        volleyball_dict = {'Blocking': 0, 'Digging': 1, 'Falling': 2, 'Jumping': 3, 'Moving': 4, 'Setting': 5,
                           'Spiking': 6, 'Standing': 7, 'Waiting': 8}
        ...
        if (self.dataset == "animalkingdom"):
            ...
            return thumos14_dict
        elif (self.dataset == "volleyball"):
            return volleyball_dict
    ...
    def get_train_loader(self, train_transform, drop_last=False):
        ...
        if (self.dataset == 'animalkingdom'):
            ...
            train_data = Charades(self.path, act_dict, total_length=self.total_length, transform=train_transform,
                                  random_shift=False, mode='train')
            sampler = RandomSampler(train_data, num_samples=2500)
            shuffle = False
        elif (self.dataset == "thumos14"):
            train_data = Thumos14(self.path, act_dict, total_length=self.total_length, transform=train_transform,
                                  mode='train')
            sampler = RandomSampler(train_data, num_samples=2500)
            shuffle = False
        elif (self.dataset == 'volleyball'):
            train_data = Volleyball(self.path, act_dict, total_length=self.total_length, transform=train_transform,
                                    mode='train')
    ...
    def get_cross_loader(self, train_transform, k=0, drop_last=False):
        """Returns the cross loader.
        """
        self._check()
        act_dict = self.get_act_dict()
        if self.dataset == 'animalkingdom':
            train_data = AnimalKingdom(self.path, act_dict, total_length=self.total_length, transform=train_transform,
                                       mode=str(k))
            sampler = None  # RandomSampler(train_data, num_samples=2500)
            shuffle = False
        elif self.dataset == 'mammalnet':
            train_data = MammalNet(self.path, act_dict, total_length=self.total_length, transform=train_transform,
                                   mode=str(k))
            sampler = None  # RandomSampler(train_data, num_samples=2500)
            shuffle = False
        elif self.dataset == 'baboonland':
    ...
    def get_test_loader(self, test_transform, drop_last=False):
        ...
        if (self.dataset == 'animalkingdom'):
            ...
            test_data = Charades(self.path, act_dict, total_length=self.total_length, transform=test_transform,
                                 mode='test')
        elif (self.dataset == 'hockey'):
            test_data = Hockey(self.path, act_dict, total_length=self.total_length, transform=test_transform,
                               mode='test', stride=45)
        elif (self.dataset == "thumos14"):
            test_data = Thumos14(self.path, act_dict, total_length=self.total_length, transform=test_transform,
                                 mode='test')
        elif (self.dataset == 'volleyball'):
            test_data = Volleyball(self.path, act_dict, total_length=self.total_length, transform=test_transform,
                                   mode='test')
        elif self.dataset == 'robot':
            test_data = RobotObservations(self.path, act_dict, total_length=self.total_length, transform=test_transform,)
        else:
            raise Exception("[ERROR] The dataset " + str(self.dataset) + " is not supported!")
