...
class RobotObservations(ActionDataset):
    def __init__(self, path, act_dict, total_length=12, annot_path='labels_robot.csv', transform=None, random_shift=False):
        # Call the parent class constructor
        super().__init__(total_length)
        ...
        try:
            ...
            print('ERROR: Could not read annotation file "{}"'.format(self.annot_path))
            raise
    ...
    def _parse_annotations(self):
        ...
        with open(self.annot_path) as f:
            ...
            for _, row in reader.iterrows():
                ovid = row['video_id']
                labels = row['labels']
                path = os.path.join(self.path, ovid)
                files = sorted(os.listdir(path))
                file_list += [files]
                count = len(files)
                labels = [int(l) for l in labels.split(',')]
